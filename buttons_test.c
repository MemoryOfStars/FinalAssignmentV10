					...
					switch(buttons[1])
					{
						...
						case 51:
							{
								...
								if(loop == 1)
								{
									for(windows_loop =0;windows_loop<9;windows_loop++)
									{
										printfWindows(file_name[windows_loop],0x0,nextWindowsArray);
									}
									nextWindowsArray[0] =  WINDOWS_POS_X0 + LCD_LINE_WIDTH_BY_PIX *2 + WINDOWS_LEFT_INSIDE_WIDTH;
								    nextWindowsArray[1] =  WINDOWS_POS_Y0;
									displayState = 1;
									currentSong = 0;
									strcat(madplay,file_name[currentSong]);
									system(madplay);
						...
						case 50:
							{//previous page
								...
								cleanScreen(0xffffffff);
								drawMP3Windows(0x00000000);
								for(windows_loop=focus*9;windows_loop<9*(focus+1)&&windows_loop<maxSong;windows_loop++)
								{
									printfWindows(file_name[windows_loop],0xff00ff,nextWindowsArray);
								}
									nextWindowsArray[0] =  WINDOWS_POS_X0 + LCD_LINE_WIDTH_BY_PIX *2 + WINDOWS_LEFT_INSIDE_WIDTH;
								    nextWindowsArray[1] =  WINDOWS_POS_Y0;
								//
						...
						case 49:
							{
								...
								for(windows_loop=focus*9;windows_loop<9*(focus+1)&&windows_loop<maxSong;windows_loop++)
								{
									printfWindows(file_name[windows_loop],0x00ff00,nextWindowsArray);
								}
									nextWindowsArray[0] =  WINDOWS_POS_X0 + LCD_LINE_WIDTH_BY_PIX *2 + WINDOWS_LEFT_INSIDE_WIDTH;
								    nextWindowsArray[1] =  WINDOWS_POS_Y0;
								//
